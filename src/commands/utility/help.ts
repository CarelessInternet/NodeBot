...
export const execute: Command['execute'] = async ({ client, interaction }) => {
	...
	const embed = new MessageEmbed()
		...
		.setDescription(
			'A list of all current available commands, including context menu commands'
		)
		.setTimestamp();
	...
	for (const category of categories) {
		...
		const fieldValue = commandsInCategory
			.reduce((acc: string[], curr) => {
				if (curr.data instanceof SlashCommandBuilder) {
	...
	const ephemeral =
		(interaction as CommandInteraction).options.getBoolean('hidden') ?? true;
